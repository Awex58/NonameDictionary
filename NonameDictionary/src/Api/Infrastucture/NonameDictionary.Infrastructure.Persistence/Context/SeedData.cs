...
namespace NonameDictionary.Infrastructure.Persistence.Context
{
    internal class SeedData
    {
        ...
        public async Task SeedAsync(IConfiguration configuration)
        {
            ...
            dbContextBuilder.UseSqlServer(configuration["NonameDictionaryDbConnectionString"]);
            var context = new NonameDictionaryContext(dbContextBuilder.Options);

            var users = GetUsers();
            var userIds = users.Select(i => i.Id);
            await context.Users.AddRangeAsync(users);

            var guids = Enumerable.Range(0, 150).Select(i => Guid.NewGuid()).ToList(); // rastgele guid oluşturup sayısı kadar atıyoruz baglantı olsun dıye
            int counter = 0;
